...
`--selftest` is green after every change.
...
gradient is gone, and those were 32 of the 21 points between them — but the
**absolute after-figure has not been re-measured on the new main**, and the row
work upstream added is not free. Re-run `qa/cpucost.sh <pid> 40` against an
installed build to get a current figure, on a machine quieter than load 26.
